...
async function generateManagementPlan(formData) {
    ...
    const businessDetail = formData.get('businessDetail') || '（未入力）';
    const hqLocation = formData.get('hqLocation') || '（未入力）';
    const otherLocations = formData.get('otherLocations') || 'なし';
    const businessArea = formData.get('businessArea') || '（未入力）';
    const employeeCount = formData.get('employeeCount') || '（未入力）';
    const futureEmployeeCount = formData.get('futureEmployeeCount') || '現状維持';
    const strengths = formData.get('strengths') || '（未入力）';
    const issues = formData.get('issues') || '（未入力）';
    const newInitiatives = formData.get('newInitiatives') || '特になし';
    const targetCustomers = formData.get('targetCustomers') || '（未入力）';
    const acquisitionChannel = formData.get('acquisitionChannel') || '（未入力）';
    const vision = formData.get('vision') || '（未入力）';
...
//# sourceMappingURL=_da723416._.js.map